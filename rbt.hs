...
data RBData a b= RBData{ rbtColor::NodeColor, rbtKey::a , rbtVal::b, rbtAggr::b } deriving Show
...
uncle = bro . dad 
colorZ = color . treeZ
goAnother z = case direction z  of 
    Tree.Left  -> goRight
    Tree.Right -> goLeft   
...
insert x m rbt = 
    let branchAggr Nothing = mempty
        ...
        transform that Nothing Nothing = RBData (rbtColor that) (rbtKey that) (rbtVal that) (rbtVal that)
        transform that left right      = RBData (rbtColor that) (rbtKey that) (rbtVal that) (rbtVal that <> branchAggr left <> branchAggr right)
...
aggregate node@(Node rbt left right) from to 
    ...
    |to < rbtKey rbt = aggregate left from to 
    |from > rbtKey rbt =  aggregate right from to 
    |from == (rbtKey rbt) = rbtVal rbt <> rightAggr right to
    |otherwise = leftAggr left from <> rbtVal rbt <> rightAggr right to

leftAggr Leaf _  = mempty
leftAggr (Node rbt left right ) from 
    |from > rbtKey rbt  = leftAggr right from
    |from == rbtKey rbt = rbtVal rbt <> aggr right
    |from < rbtKey rbt  = leftAggr left from <>  rbtVal rbt <> aggr right

rightAggr Leaf _ = mempty
rightAggr (Node rbt left right) to
    |to < rbtKey rbt = rightAggr left to
    |to >= rbtKey rbt = aggr left <> rbtVal rbt <> rightAggr right to

fold::b->(a->b->b->b)->RBT a c->b
fold s f Leaf = s
fold s f (Node rbx left right ) = f (rbtKey rbx) (fold s f left) (fold s f right) 
